...
// For security, consider storing these in environment variables
const CLOUDINARY_CLOUD_NAME = "duqlw2kta";
...
const VerificationModal = ({ user, onClose }) => {
  ...
  const getIdOptions = (countryCode) => {
    ...
    const countrySpecific = {
      ...
      Ghana: [{ value: "ghanacard", label: "Ghana Card", requiresBack: true }],
      Kenya: [
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let backUrl = null;
      if (idBack) {
        backUrl = await uploadToCloudinary(idBack);
      }

      // Step 2: Send the Cloudinary URLs to your backend
      ...
      await axios.post(`${Api}/client/upload`, verificationData);
        ...
        <div className="flex-1 px-6 py-4 overflow-y-auto">
          ...
          <div
            ...
            style={{
              ...
              border: "1px solid #18ffc8",
            }}
          >
            <FaGlobe className="mt-1 mr-3" style={{ color: "#18ffc8" }} />
            <div>
              <h3 className="font-bold mb-1" style={{ color: "#18ffc8" }}>
                Verification for {userCountry}
              </h3>
              <p className="text-sm" style={{ color: "#efefef" }}>
                Based on your country code {user.countryCode}, we've provided ID
                options commonly used in {userCountry}.
              </p>
            </div>
          </div>
          ...
          <div
            className="mb-6 p-4 rounded-lg flex items-start"
            style={{
              backgroundColor: "rgba(254, 169, 42, 0.1)",
              border: "1px solid #fea92a",
            }}
          >
            <FaInfoCircle className="mt-1 mr-3" style={{ color: "#fea92a" }} />
            <div>
              <h3 className="font-bold mb-1" style={{ color: "#fea92a" }}>
                Important Information
              </h3>
              <p className="text-sm" style={{ color: "#efefef" }}>
                The name on your ID document must exactly match the name on your
                account (
                <span style={{ color: "#f57cff" }}>
                  {user.firstname} {user.lastname}
                </span>
                ) to avoid rejection. Ensure your document is valid and clearly
                visible.
              </p>
            </div>
          </div>
          ...
          {uploadComplete ? (
            <div className="text-center py-8">
              ...
              <p className="mb-6" style={{ color: "#efefef" }}>
                Your ID documents have been uploaded successfully. Our team will
                review them and update your verification status within 24-48
                hours.
              </p>
              <button
                onClick={onClose}
                  ...
                  {idOptions.map((option) => (
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        >
                          {selectedIdType === option.value && (
                            <FaCheck size={10} style={{ color: "#09100d" }} />
                          )}
                        </div>
